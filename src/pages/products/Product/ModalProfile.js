...
const ModalProfile = ({ isOpen, closeModal, selectedItem }) => {
    ...
    const nameFileRegex = (selectedItem) => {
        ...
        if (selectedItem.hinhAnh !== null) {
            let myRE = /(https:\/\/firebasestorage.googleapis.com\/v0\/b\/fam-shop-4fd26.appspot.com\/o\/product%2F)(.*)(\?.*)/g
        ...
        <Modal show={isOpen} size='md'>
            ...
            <ModalBody className="text-left m-3">
                <Formik
                    ...
                    onSubmit={(values) => {
                        const response = SanPhamApi.uploadProfileImage(values.file, selectedItem.maSP)
                        response.then(async (rs) => {
                            const res = await SanPhamApi.getById(selectedItem.maSP);
